...
describe('Unit test: Basic LyraVault flow', async () => {
  ...
  describe('owner settings', async () => {
    ...
    it('owner should be able to set a new management fee', async () => {
      ...
      const weeklyFee = ethers_1.BigNumber.from(managementFee)
        .mul(constants_1.FEE_MULTIPLIER)
        .div(constants_1.WEEKS_PER_YEAR);
    ...
    it("should revert when trying to set a mangement fee that's too high", async () => {
      await (0, chai_1.expect)(
        vault.connect(owner).setManagementFee(100 * constants_1.FEE_MULTIPLIER),
    ...
    it("should revert when trying to set a performance fee that's too high", async () => {
      await (0, chai_1.expect)(
        vault.connect(owner).setPerformanceFee(100 * constants_1.FEE_MULTIPLIER),
  ...
  describe('set strategy', async () => {
    it('should revert if called by non-owner', async () => {
      await (0, chai_1.expect)(
        vault.connect(anyone).setStrategy(hardhat_1.ethers.constants.AddressZero),

...
xdescribe("Short Put Delta Strategy integration test", async () => {
  // mocked tokens
  let susd: MockERC20;
  let seth: MockERC20;
  ...
  describe("start the first round", async () => {
    ...
    it("should be able to close closeRound after settlement", async () => {
      ...
      const susdInStrategyAfterSettlement = await susd.balanceOf(
        strategy.address
      );

      // collateral should be back into the strategy after settlement
      expect(susdInStrategyAfterSettlement.sub(susdInStrategyBefore).gt(0)).to
        .be.true;
      ...
      expect(
        susdInVaultAfter.sub(usdInVaultBefore).eq(susdInStrategyAfterSettlement)
      );
    });
  });
  describe("start round 2", async () => {
    ...
    beforeEach(async () => {
      ...
      [strikePrice, expiry] =
        await lyraTestSystem.optionMarket.getStrikeAndExpiry(strikes[2]);
    ...
    it("should revert when trying to reduce a safe position", async () => {
      ...
      await expect(
        vault.connect(randomUser).reducePosition(positionId, toBN("10000"))
    ...
    it("partially reduce position if unsafe position + delta is in range", async () => {
      ...
      await vault
        .connect(randomUser)
        .reducePosition(positionId, fullCloseAmount.div(2));
      const postReduceBal = await susd.balanceOf(strategy.address);
      expect(postReduceBal).to.be.lt(preReduceBal);
    });

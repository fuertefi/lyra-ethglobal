...
describe('Hack Money Strategy integration test', async () => {
  ...
  const boardParameter = {
    ...
    baseIV: '0.85',
    strikePrices: ['1500', '1600', '1700', '1750', '1800', '1900', '2000', '2100', '2200'],
    skews: ['1.23', '1.15', '1.05', '1.03', '1', '0.97', '0.96', '0.95', '0.95'],
  };
  ...
  before('deploy strategy', async () => {
    ...
    strategy = (await (
      ...
      .connect(manager)
      .deploy(vault.address, TestSystem.OptionType.SHORT_CALL_BASE)) as HackMoneyStrategy;
  });
  ...
  describe('start the first round', async () => {
    ...
    it('should trade when called first time', async () => {
      ...
      console.log('strategySETHBalanceBefore:', ethers.utils.formatEther(strategySETHBalanceBefore));
      const strategySUSDBalanceBefore = await susd.balanceOf(strategy.address);
      console.log('strategySUSDBalanceBefore:', ethers.utils.formatEther(strategySUSDBalanceBefore));

      const tradeTransaction = await vault.connect(randomUser).trade(strategyDetail.size);
      ...
      const tradeEventLength = tradeTransactionReceipt.events?.length ? tradeTransactionReceipt.events?.length - 1 : 0;
      //const tradeEventArgs = tradeTransactionReceipt.events?.at(110)?.args;
      const tradeEventArgs = tradeTransactionReceipt.events?.at(tradeEventLength)?.args;
      ...
      const premiumExchangeValue = ethers.utils.formatEther(tradeEventArgs?.premiumExchangeValue);
      ...
      console.log('strategySETHBalanceAfter:', ethers.utils.formatEther(strategySETHBalanceAfter));
      const strategySUSDBalanceAfter = await susd.balanceOf(strategy.address);
      console.log('strategySUSDBalanceAfter:', ethers.utils.formatEther(strategySUSDBalanceAfter));

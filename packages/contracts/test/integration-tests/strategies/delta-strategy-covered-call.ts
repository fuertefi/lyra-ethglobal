...
xdescribe("Covered Call Delta Strategy integration test", async () => {
  ...
  describe("start the first round", async () => {
    ...
    it("should revert when trying to start with invalid boardId", async () => {
      await expect(vault.connect(manager).startNextRound(0)).to.be.revertedWith(
        "timestamp expired"
      );
    });
    ...
    it("should revert when trying to update strategy detail mid-round", async () => {
      await expect(
        strategy.connect(manager).setStrategyDetail(strategyDetail)
      ).to.revertedWith("cannot change strategy if round is active");
    });
    it('will not trade when delta is out of range"', async () => {
      // 2500 is a bad strike because delta is close to 1
      await expect(
        vault.connect(randomUser).trade(strikes[0])
      ).to.be.revertedWith("invalid strike");

      // 3000 is a bad strike because delta is close to 0.5
      await expect(
        vault.connect(randomUser).trade(strikes[1])
      ).to.be.revertedWith("invalid strike");

      // 3200 is a bad strike (delta is close to 0.34)
      await expect(
        vault.connect(randomUser).trade(strikes[2])
      ).to.be.revertedWith("invalid strike");
    });
    ...
    it("should revert when min premium < premium calculated with min vol", async () => {
      ...
      await expect(
        vault.connect(randomUser).trade(strikes[4])
      ).to.be.revertedWith("TotalCostOutsideOfSpecifiedBounds");
    ...
    it("should trade when delta and vol are within range", async () => {
      const strikeObj = await strikeIdToDetail(
      ...
      expect(
        vaultStateBefore.lockedAmountLeft
          .sub(vaultStateAfter.lockedAmountLeft)
          .eq(collateralToAdd)
      ).to.be.true;
      // check that we receive sUSD
      expect(strategySUDCBalanceAfter.sub(strategySUSDBalance).gt(0)).to.be
        .true;
      ...
      const [position] = await lyraTestSystem.optionToken.getOptionPositions([
        positionId,
      ]);
    ...
    it("should revert when user try to trigger another trade during cooldown", async () => {
      await expect(
        vault.connect(randomUser).trade(strikes[3])
      ).to.be.revertedWith("min time interval not passed");
    });
    ...
    it("should be able to trade again after time interval", async () => {
      ...
      const [positionBefore] =
        await lyraTestSystem.optionToken.getOptionPositions([positionId]);
      ...
      const [positionAfter] =
        await lyraTestSystem.optionToken.getOptionPositions([positionId]);
      expect(
        positionAfter.amount.sub(positionBefore.amount).eq(strategyDetail.size)
      ).to.be.true;
    });
    ...
    it("should revert when closeRound is called before options are settled", async () => {
      await expect(vault.closeRound()).to.be.revertedWith(
        "cannot clear active position"
      );
    });
    it("should revert when returnFundsAndClearStrikes is called by non-vault address", async () => {
      await expect(strategy.returnFundsAndClearStrikes()).to.be.revertedWith(
        "only Vault"
      );
    });
  ...
  describe("start round 2", async () => {
    ...
    before("create new board", async () => {
      await TestSystem.marketActions.createBoard(
        lyraTestSystem,
        boardParameter
      );
      const boards = await lyraTestSystem.optionMarket.getLiveBoards();
    ...
    before("make a trade", async () => {
      ...
      [strikePrice, expiry] =
        await lyraTestSystem.optionMarket.getStrikeAndExpiry(strikes[3]);
      ...
      const strategySUDCBalanceAfter = await susd.balanceOf(strategy.address);
      expect(strategySUDCBalanceAfter.sub(strategySUSDBalanceBefore).gt(0)).to
        .be.true;
    });
    ...
    it("should revert when trading with old strike", async () => {
      ...
      await expect(vault.connect(randomUser).trade(1)).to.be.revertedWith(
        "invalid strike"
      );
    });

    it("should revert when trying to reduce a safe position", async () => {
      let fullCloseAmount = await strategy.getAllowedCloseAmount(
        position,
        strikePrice,
        expiry
      );
      ...
      await expect(
        vault.connect(randomUser).reducePosition(positionId, toBN("0.1"))
      ...
      fullCloseAmount = await strategy.getAllowedCloseAmount(
        position,
        strikePrice,
        expiry
      );
    ...
    it("reduce full position if unsafe position + delta is in range", async () => {
      ...
      const fullCloseAmount = await strategy.getAllowedCloseAmount(
        position,
        strikePrice,
        expiry.sub(10)
      ...
      await vault
        .connect(randomUser)
        .reducePosition(positionId, fullCloseAmount);
    ...
    it("partially reduce position if unsafe position + delta is in range", async () => {
      ...
      await vault
        .connect(randomUser)
        .reducePosition(positionId, fullCloseAmount.div(6));
      const postReduceBal = await susd.balanceOf(strategy.address);
      expect(postReduceBal).to.be.lt(preReduceBal);
    });
    ...
    it("revert reduce position if unsafe position + close amount too large", async () => {
      ...
      await expect(
        vault
          .connect(randomUser)
          .reducePosition(positionId, fullCloseAmount.mul(2))
      ).to.be.revertedWith("amount exceeds allowed close amount");
    });
    ...
    it("partially reduce position with force close if delta out of range", async () => {
      ...
      await vault
        .connect(randomUser)
        .reducePosition(positionId, fullCloseAmount.div(15));
      const postReduceBal = await susd.balanceOf(strategy.address);
      expect(postReduceBal).to.be.lt(preReduceBal);
    });
  });
  describe("end round2: assuming position got liquidated", async () => {
    ...
    it("liquidating position ", async () => {
      ...
      await lyraTestSystem.optionMarket
        .connect(randomUser)
        .liquidatePosition(positionId, randomUser.address);

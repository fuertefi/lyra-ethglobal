...
export const Main = () => {
    ...
    useEffect(() => {
        ...
        let timer = setTimeout(() => setStrategyDetails(getCurrentStrategy()), delayMilliseconds);
        return () => {
            clearTimeout(timer);
        };
    }, []);
        ...
        <ContentColumns>
            <div className="left-section">
                ...
                <StyledVaultDescription link="https://github.com/fuertefi/core/pull/8">
                    T-YVUSDC-P-ETH earns yield on its USDC deposits by running a weekly automated ETH put-selling
                    strategy, where the put options are collateralized by yvUSDC. The vault reinvests the yield it earns
                    back into the strategy, effectively compounding the yields for depositors over time.
                </StyledVaultDescription>
                {!strategyDetails && <LoadingOutlined />}
                {strategyDetails && <StrategyDetailsPanel strategyDetails={strategyDetails} />}
            </div>

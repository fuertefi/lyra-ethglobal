...
contract HackMoneyStrategy is HackMoneyStrategyBase, IHackMoneyStrategy {
  ...
  constructor(HackMoneyVault _vault, OptionType _optionType) HackMoneyStrategyBase(_vault, _optionType) {}
  ...
  // uint exchangeValue
  {
    ...
    (positionId1, positionId2, premiumReceived, collateralToAdd, premiumExchangeValue) = _tradeOptions();
  }
    ...
    )
  {
    ...
    OptionPosition memory _position1 = _getPositions(_toDynamic(positionId1))[0];
    ...
    OptionPosition memory _position2 = _getPositions(_toDynamic(positionId2))[0];
    ...
    (, , additionalPremium, premiumExchangeValue) = _tradePremiums(premiumReceived, collateralToAdd1, collateralToAdd2);
    ...
    OptionPosition memory position2 = _getPositions(_toDynamic(positionId2))[0];
    ...
    )
  {
    ...
    (Strike memory strike1, Strike memory strike2) = _getTradeStrikes();
    uint premiumReceived1;
    (positionId1, premiumReceived1) = _sellPremiums(strike1, sellAmount, collateralToAdd1);
    uint premiumReceived2;
    (positionId2, premiumReceived2) = _sellPremiums(strike2, sellAmount, collateralToAdd2);
   ...
   * @return premiumReceived
   */
  function _sellStrike(Strike memory strike, uint setCollateralTo) internal returns (uint, uint) {
  ...
  function _sellPremiums(
    ...
    // get minimum expected premium based on minIv
    uint minExpectedPremium = _getPremiumLimit(strike, strategyDetail.minVol, size);
  ...
  function _getTradeStrikes() internal view returns (Strike memory smallStrike, Strike memory bigStrike) {
    ...
    console.log("finding small strike");
    for (uint i = 1; i < strikeIds.length - 1; i++) {
      ...
      console.log(currentStrike.strikePrice);
      (uint currentDeltaGap, ) = _getDeltaGap(currentStrike, true);
      console.log(currentDeltaGap);
    ...
    console.log("finding big strike");
  ...
  function _exchangePremiums(uint size) internal returns (uint baseReceived) {
    ...
    uint minQuoteExpected = size.divideDecimal(exchangeParams.spotPrice).multiplyDecimal(

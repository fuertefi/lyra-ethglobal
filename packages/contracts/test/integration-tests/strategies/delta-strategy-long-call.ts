...
xdescribe("Long Call Strategy integration test", async () => {
  // mocked tokens
  let susd: MockERC20;
  let seth: MockERC20;
  ...
  before("deploy strategy", async () => {
    strategy = (await (
      ...
      .deploy(
        vault.address,
        TestSystem.OptionType.LONG_CALL,
        lyraTestSystem.GWAVOracle.address
      )) as DeltaLongStrategy;
  });
  ...
  describe("setStrategy", async () => {
    it("setting strategy should correctly update strategy variables", async () => {
      ...
      expect(newStrategy.minTradeInterval).to.be.eq(
        strategyDetail.minTradeInterval
      );
    });
  ...
  describe("start round 2", async () => {
    ...
    it("should revert when trying to reduce a position", async () => {
      await expect(
        vault.connect(randomUser).reducePosition(positionId, 0)

...
contract BaseVault is ReentrancyGuard, Ownable, ERC20, Initializable {
   ...
   */
  function withdrawInstantly(uint amount) external nonReentrant {
    ...
    uint receiptAmount = depositReceipt.amount;
    require(receiptAmount >= amount, "Exceed amount");

    // Subtraction underflow checks already ensure it is smaller than uint104
    depositReceipt.amount = uint104(receiptAmount - amount);
    vaultState.totalPending = uint128(uint(vaultState.totalPending) - amount);

...
xdescribe("Unit test: Basic LyraVault flow", async () => {
  // contract instances
  let vault: LyraVault;
  ...
  describe("owner settings", async () => {
    it("shoud revert when setting cap as 0", async () => {
      await expect(vault.connect(owner).setCap(0)).to.be.revertedWith(
        "!newCap"
      );
    });
    ...
    it("should revert when trying to set a mangement fee that's too high", async () => {
      await expect(
        vault.connect(owner).setManagementFee(100 * FEE_MULTIPLIER)
    ...
    it("should revert when trying to set a performance fee that's too high", async () => {
      await expect(
        vault.connect(owner).setPerformanceFee(100 * FEE_MULTIPLIER)
      ).to.be.revertedWith("Invalid performance fee");
    });
    it("should revert when trying to set a invalid feeRecipient high", async () => {
      await expect(
        vault.connect(owner).setFeeRecipient(ethers.constants.AddressZero)
    ...
    it("should revert if trying to set the same feeRecipient as the existing one", async () => {
      await expect(
        vault.connect(owner).setFeeRecipient(feeRecipient.address)
      ).to.be.revertedWith("Must be new feeRecipient");
    });
  });
  ...
  describe("set strategy", async () => {
    it("should revert if called by non-owner", async () => {
      await expect(
        vault.connect(anyone).setStrategy(ethers.constants.AddressZero)
  ...
  describe("basic deposit", async () => {
    it("should revert if trying to deposit 0", async () => {
      await expect(vault.connect(anyone).deposit(0)).to.be.revertedWith(
        "!amount"
      );
    });
    it("should revert if trying to use depositFor with amount = 0", async () => {
      await expect(
        vault.connect(anyone).depositFor(0, depositor.address)
      ).to.be.revertedWith("!amount");
    });
    it("should revert if trying to deposit to a 0 address", async () => {
      await expect(
        vault.connect(anyone).depositFor(1, ethers.constants.AddressZero)
    ...
    it("should deposit seth into the contract", async () => {
      ...
      expect(
        newReceipt.unredeemedShares.sub(initReceipt.unredeemedShares)
      ).to.be.eq(0);
    });
    it("should revert if deposit amount exceed the cap", async () => {
      ...
      await seth
        ...
        .approve(vault.address, ethers.constants.MaxUint256);
      await expect(
        vault.connect(depositor).deposit(depositAmount)
  ...
  describe("start the second round", async () => {
    ...
    it("should revert if startNextRound is called by arbitrary user", async () => {
      ...
      await expect(
        vault.connect(anyone).startNextRound(wrongRoundId)
      ).to.be.revertedWith("Ownable: caller is not the owner");
    });
  ...
  describe("trade flow tests", async () => {
    ...
    it("should successfully trade with mocked amount", async () => {
      await mockedStrategy.setMockedTradeAmount(
        roundPremiumSUSD,
        collateralAmount
      );
  ...
  describe("rollover", async () => {
    ...
    it("should roll into the next round and pay the recipient fees", async () => {
      ...
      const roundPerformanceFee = roundPremiumInEth
        .mul(performanceFee)
      ...
      expect(recipientBalanceAfter.sub(recipientBalanceBefore)).to.be.eq(
        roundManagementFee.add(roundPerformanceFee)
      );
    });
  });
  ...
  describe("misc testing", async () => {
    it("setting new strategy should reset allowance", async () => {
      ...
      const allowanceAfter = await seth.allowance(
        vault.address,
        mockedStrategy.address
      );

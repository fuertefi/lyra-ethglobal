...
xdescribe("Unit test: share calculating for pending deposit and withdraw", async () => {
  // contract instances
  let vault: LyraVault;
  ...
  before("prepare mocked contracts", async () => {
    const MockOptionMarketFactory = await ethers.getContractFactory(
      "MockOptionMarket"
    );
    mockedMarket = (await MockOptionMarketFactory.deploy()) as MockOptionMarket;

    const MockSynthetixFactory = await ethers.getContractFactory(
      "MockSynthetix"
    );
  ...
  describe("round 1", async () => {
    describe("basic deposit and withdraw in round 1.", async () => {
      it("should return 0 share before any deposit", async () => {
        const { heldByAccount, heldByVault } = await vault.shareBalances(
          depositor.address
        );
      ...
      it("should deposit seth into the contract and update the receipt", async () => {
        ...
        expect(newState.totalPending.sub(initState.totalPending)).to.be.eq(
          depositAmount
        );
        expect(newReceipt.amount.sub(initReceipt.amount)).to.be.eq(
          depositAmount
        );
        expect(newReceipt.unredeemedShares).to.be.eq(
          initReceipt.unredeemedShares
        );
      });
      it("should be able to use depositFor to do the same thing for depositor", async () => {
        ...
        await vault
          .connect(anyone)
          .depositFor(depositAmount, depositor.address);
      ...
      it("should revert when calling redeem becuase depositor has no unreedemed shares", async () => {
        const sharesToWithdraw = depositAmount;
        await expect(
          vault.connect(depositor).redeem(sharesToWithdraw)
    ...
    describe("trade during first round", async () => {
      it("should revert because the first round is not started yet", async () => {
        ...
        await expect(vault.trade(strikeIdToTrade)).to.be.revertedWith(
          "round closed"
        );
      });
    });
  ...
  describe("round 2: vault makes profit", async () => {
    ...
    describe("redeem shares", async () => {
      it("should revert while redeeming 0 shares", async () => {
        await expect(vault.connect(depositor).redeem(0)).to.be.revertedWith(
          "!numShares"
        );
      });
      it("should has some share balance after rollover", async () => {
        const { heldByVault, heldByAccount } = await vault.shareBalances(
          depositor.address
        );
        expect(heldByAccount).to.be.eq(0);
    ...
    describe("initiate withdraw", async () => {
      ...
      it("should be able to initiate a withdraw, this will trigger a max redeem", async () => {
        const totalShares = await vault.shares(depositor.address);
        const totalSharesToInitiateWithdraw = totalShares.div(
          1 / initiateWithdrawSharePercentage
        );

        // only que withraw 1/8 of total shares
        const sharesToInitaiteWithrawFirst =
          totalSharesToInitiateWithdraw.div(2);
        sharesToInitaiteWithrawSecond = totalSharesToInitiateWithdraw.sub(
          sharesToInitaiteWithrawFirst
        );

        await vault
          .connect(depositor)
          .initiateWithdraw(sharesToInitaiteWithrawFirst);

        const { heldByVault, heldByAccount } = await vault.shareBalances(
          depositor.address
        );
        expect(heldByAccount.add(sharesToInitaiteWithrawFirst)).to.be.eq(
          totalShares
        );
      ...
      it("should be able to que more withdraw share amount", async () => {
        ...
        const withdrawReceiptBefore = await vault.withdrawals(
          depositor.address
        );
        await vault
          .connect(depositor)
          .initiateWithdraw(sharesToInitaiteWithrawSecond);
        const withdrawReceiptAfter = await vault.withdrawals(depositor.address);

        expect(
          withdrawReceiptAfter.shares.sub(withdrawReceiptBefore.shares)
        ).to.be.eq(sharesToInitaiteWithrawSecond);
        expect(withdrawReceiptAfter.round).to.be.eq(
          withdrawReceiptBefore.round
        );
      });
    });
    ...
    describe.skip("stimulate trade", async () => {
      ...
      it("should successfully trade", async () => {
        ...
        expect(
          sethBefore.sub(collateralAmount).add(round3PremiumInEth)
        ).to.be.eq(sethAfter);
      });
    });
    ...
    describe("settle and close", async () => {
      ...
      it("should revert if trying to completeWithdraw", async () => {
        await expect(
          vault.connect(depositor).completeWithdraw()
        ).to.be.revertedWith("Round in progress");
      });
  ...
  describe("round 3: vault lose money", async () => {
    ...
    describe.skip("stimulate a trade", async () => {
      ...
      it("should successfully trade", async () => {
        ...
        await vault.trade(0);
        const sethAfter = await seth.balanceOf(vault.address);
        expect(sethBefore.sub(collateralAmount).add(premiumInEth)).to.be.eq(
          sethAfter
        );
      });
    });
  ...
  describe.skip("round 4", async () => {
    ...
    it("should revert when trying to complete the withdraw, because the collateral is 0", async () => {
      await expect(vault.connect(shrimp).completeWithdraw()).to.be.revertedWith(
        "!withdrawAmount"
      );
    });
  });

...
contract HackMoneyStrategy is HackMoneyStrategyBase, IHackMoneyStrategy {
    ...
    )
  {
    ...
    premiumReceived = premiumReceived1 + premiumReceived2 + additionalPremium;
  }
    ...
    )
  {
    ...
    (Strike memory strike1, Strike memory strike2) = _getTradeStrikes();
    uint premiumReceived1;
    (positionId1, premiumReceived1) = _sellPremiums(strike1, size, collateralToAdd1, lyraRewardRecipient);
    uint premiumReceived2;
    (positionId2, premiumReceived2) = _sellPremiums(strike2, size, collateralToAdd2, lyraRewardRecipient);
   ...
   */
  function _getPositionSize(uint strikePrice, uint collateralToLock) internal view returns (uint size) {
    // calculate required collat based on collatBuffer and collatPercent
    //fullCollat = _isBaseCollat() ? amount : amount.multiplyDecimal(strikePrice);
    size = _isBaseCollat() ? collateralToLock : collateralToLock.divideDecimal(strikePrice);
  }
  ...
  function _exchangePremiums() internal returns (uint baseReceived) {
    ...
    baseReceived = exchangeFromExactQuote(quoteBal, minQuoteExpected);
  }
}

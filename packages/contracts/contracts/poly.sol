...
interface ICollateralShort {
    ...
    function repayWithCollateral(uint id, uint repayAmount)
        external
        returns (uint short, uint collateral);

    function draw(uint id, uint amount)
        external
        returns (uint short, uint collateral);
    ...
    function getShortAndCollateral(address account, uint id)
        external
        view
        returns (uint short, uint collateral);
}
...
interface ILyraGlobals {
    ...
    function setTradingCutoff(address _contractAddress, uint _tradingCutoff)
        external;

    function setOptionPriceFeeCoefficient(
        address _contractAddress,
        uint _optionPriceFeeCoefficient
    ) external;

    function setSpotPriceFeeCoefficient(
        address _contractAddress,
        uint _spotPriceFeeCoefficient
    ) external;

    function setVegaFeeCoefficient(
        address _contractAddress,
        uint _vegaFeeCoefficient
    ) external;

    function setVegaNormFactor(address _contractAddress, uint _vegaNormFactor)
        external;

    function setStandardSize(address _contractAddress, uint _standardSize)
        external;

    function setSkewAdjustmentFactor(
        address _contractAddress,
        uint _skewAdjustmentFactor
    ) external;

    function setRateAndCarry(address _contractAddress, int _rateAndCarry)
        external;

    function setMinDelta(address _contractAddress, int _minDelta) external;

    function setVolatilityCutoff(
        address _contractAddress,
        uint _volatilityCutoff
    ) external;
    ...
    function getExchangeGlobals(
        address _contractAddress,
        ExchangeType exchangeType
    ) external view returns (ExchangeGlobals memory exchangeGlobals);
...
interface ILiquidityPool {
    ...
    function startRound(uint lastMaxExpiryTimestamp, uint newMaxExpiryTimestamp)
        external;
    ...
    function getTotalPoolValueQuote(uint basePrice, uint usedDeltaLiquidity)
        external
        view
        returns (uint);

    function getLiquidity(uint basePrice, ICollateralShort short)
        external
        view
        returns (Liquidity memory);

    function transferQuoteToHedge(
        ILyraGlobals.ExchangeGlobals memory exchangeGlobals,
        uint amount
...
interface IOptionMarket {
    ...
    function getBoardListings(uint boardId)
        external
        view
        returns (uint[] memory);
...
contract PolynomialCoveredCall is
    IPolynomialCoveredCall,
    ReentrancyGuard,
    Auth,
    Pausable
{
    ...
    ERC20 public constant LYRA_TOKEN =
        ERC20(0x50c5725949A6F0c72E6C4a641F24049A917DB0Cb);

    /// @notice An instance of LYRA distributor
    ILyraDistributor public constant LYRA_CLAIMER =
        ILyraDistributor(0x0BFb21f64E414Ff616aC54853e52679EEDB22Dd2);
    ...
    /// @param _amt Amount of UNDERLYING tokens to deposit
    function deposit(address _user, uint256 _amt)
        external
        override
        nonReentrant
        whenNotPaused
    ...
    function requestWithdraw(uint256 _shares) external override nonReentrant {
        ...
        if (
            userInfo.depositRound < currentRound && userInfo.pendingDeposit > 0
        ) {
    ...
    function setFees(uint256 _perfomanceFee, uint256 _managementFee)
        external
        requiresAuth
    {
    ...
    function startNewRound(uint256 _listingId)
        external
        requiresAuth
        nonReentrant
    {
        /// Check if listing ID is valid & last round's expiry is over
        (, uint256 strikePrice, , , , , , uint256 boardId) = LYRA_MARKET
            .optionListings(_listingId);
        ...
        if (currentRound > 0) {
            ...
            if (usedFunds > 0) {
                ...
                LYRA_MARKET.settleOptions(
                    currentListingId,
                    IOptionMarket.TradeType.SHORT_CALL
                );
                ...
                if (collateralWithdrawn == usedFunds) {
                    uint256 currentRoundManagementFees = collateralWithdrawn
                        .fmul(managementFee, WEEKS_PER_YEAR);
                    uint256 currentRoundPerfomanceFee = premiumCollected.fmul(
                        performanceFee,
                        WEEKS_PER_YEAR
                    );
                    totalFees =
                        currentRoundManagementFees +
                        currentRoundPerfomanceFee;
            ...
            uint256 fundsPendingWithdraws = pendingWithdraws.fmul(
                newIndex,
                1e18
            );
            totalFunds =
    ...
    function sellOptions(uint256 _amt)
        external
        onlyKeeper
        nonReentrant
        whenNotPaused
    {
        ...
        (, , uint256 initSkew, , , , , uint256 boardId) = LYRA_MARKET
            .optionListings(currentListingId);
        ...
        uint256 totalCost = LYRA_MARKET.openPosition(
            currentListingId,
            tradeType,
            _amt
        );
        ...
        (, , uint256 finalSkew, , , , , ) = LYRA_MARKET.optionListings(
            currentListingId
        );
    ...
    function _depositForRoundZero(address _user, uint256 _amt) internal {
        ...
        require(
            UNDERLYING.balanceOf(address(this)) <= vaultCapacity,
            "CAPACITY_EXCEEDED"
        );
        ...
        require(
            userInfo.totalShares <= userDepositLimit,
    ...
    function _deposit(address _user, uint256 _amt) internal {
        ...
        pendingDeposits += _amt;
        require(
            totalFunds + pendingDeposits < vaultCapacity,
            "CAPACITY_EXCEEDED"
        );
        ...
        uint256 totalBalance = userInfo.pendingDeposit +
            userInfo.totalShares.fmul(
                performanceIndices[currentRound - 1],
                1e18
            );
        require(
            totalBalance <= userDepositLimit,
            "USER_DEPOSIT_LIMIT_EXCEEDED"
        );
    }
}
